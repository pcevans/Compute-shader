...
bool RenderTextureClass::Initialize_depth(ID3D11Device* device, HWND hwnd, int width, int height)
	{
	if (m_renderTargetTexture2D) return TRUE;
	...
	result = device->CreateTexture2D(&texDesc, NULL, &m_renderTargetTexture2D);
	if (FAILED(result))
		{
		return false;
		}
	...
	result = device->CreateShaderResourceView(m_renderTargetTexture2D, &srvDesc, &m_shaderResourceView);
	if (FAILED(result))
		{
		return false;
		}
	...
	dsvDesc.Flags = 0;
	result = device->CreateDepthStencilView(m_renderTargetTexture2D, &dsvDesc, &m_DepthStencilView);
	if (FAILED(result))
		{
		return false;
		}
...
bool RenderTextureClass::Initialize_3DTex(ID3D11Device* device, int width, int height, int depth, bool uav_, DXGI_FORMAT format, bool mipmaps)
	...
	if (!uav_)
		...
		result = device->CreateRenderTargetView(m_renderTargetTexture3D, &renderTargetViewDesc, &m_renderTargetView);
		if (FAILED(result))			return false;
		}
...
//------------------------------------
bool RenderTextureClass::Initialize_2DTex(ID3D11Device* device, HWND hwnd, int width, int height, bool uav_, DXGI_FORMAT format, bool mipmaps)
	{
	...
	// Create the render target texture.
	result = device->CreateTexture2D(&textureDesc, NULL, &m_renderTargetTexture2D);
	if (FAILED(result))
		{
		return false;
		}
	...
	// Create the render target view.
	result = device->CreateRenderTargetView(m_renderTargetTexture2D, &renderTargetViewDesc, &m_renderTargetView);
	if (FAILED(result))
		{
		return false;
		}
	...
	// Create the shader resource view.
	result = device->CreateShaderResourceView(m_renderTargetTexture2D, &shaderResourceViewDesc, &m_shaderResourceView);
	if (FAILED(result))
		{
		return false;
		}
	...
	if (uav)
		...
		result = device->CreateUnorderedAccessView(m_renderTargetTexture2D, &descUAV, &m_pUAVs);
		if (FAILED(result))
			return false;
...
bool RenderTextureClass::Initialize_1DTex(ID3D11Device* device, HWND hwnd, int width, bool uav_, DXGI_FORMAT format, bool mipmaps)
{
	if (m_renderTargetView) return TRUE;
	...
	// Create the render target texture.
	result = device->CreateTexture1D(&textureDesc, NULL, &m_renderTargetTexture1D);
	if (FAILED(result))
	{
		return false;
	}
	...
	// Create the render target view.
	result = device->CreateRenderTargetView(m_renderTargetTexture1D, &renderTargetViewDesc, &m_renderTargetView);
	if (FAILED(result))
	{
		return false;
	}
	...
	//if (mipmaps)
	//shaderResourceViewDesc.Texture1D.MipLevels = 8;



	// Create the shader resource view.
	result = device->CreateShaderResourceView(m_renderTargetTexture1D, &shaderResourceViewDesc, &m_shaderResourceView);
	if (FAILED(result))
	{
		return false;
	}
	...
	if (uav)
	{
		...
		result = device->CreateUnorderedAccessView(m_renderTargetTexture1D, &descUAV, &m_pUAVs);
		if (FAILED(result))
			return false;
...
void RenderTextureClass::Shutdown()
	...
	if (m_renderTargetTexture2D)
		{
		m_renderTargetTexture2D->Release();
		m_renderTargetTexture2D = 0;
		}
